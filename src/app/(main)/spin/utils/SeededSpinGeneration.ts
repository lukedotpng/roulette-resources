...
export function GenerateSpin(mission: Mission, seededRandom: Rand): Spin {
    ...
    for (let i = 0; i < targets.length; i++) {
        let randomIndex = 0;
        do {
            randomIndex = Math.floor(seededRandom.next() * targets.length);
...
function GetRandomDisguise(
    ...
    let disguise =
        disguiseList[Math.floor(seededRandom.next() * disguiseList.length)];
...
function GetRandomCondition(
    ...
    let conditionType: KillMethodType =
        conditionTypeList[Math.floor(seededRandom.next() * conditionTypeSize)];
    ...
    // Add "remote_" , "impact_", or "loud_" prefix if condition is explosive
        ...
        if (noMeleesOrRemoteForced) {
            ...
            if (
                ...
                modifierPrefix =
                    seededRandom.next() > 0.5 ? "remote_" : "loud_";
            } else {
                modifierPrefix =
                    EXPLOSIVE_MODIFIER_PREFIX[
                        Math.floor(
                            seededRandom.next() *
                                EXPLOSIVE_MODIFIER_PREFIX.length,
                        )
                    ];
            }
