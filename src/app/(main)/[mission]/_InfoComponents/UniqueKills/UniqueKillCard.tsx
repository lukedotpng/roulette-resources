        ...
        <div className="w-full min-w-full flex-1 rounded-xl border-4 border-zinc-500 bg-white text-zinc-900 md:max-w-[50rem] md:min-w-[25rem]">
            <div className="flex">
                {targets.map((target) => (
                    <button
                        key={target}
                        data-active={activeTargetId === target}
                        className="w-full flex-1 border-x-1 border-zinc-900 p-1 text-center font-bold first:rounded-tl-lg first:border-l-0 last:rounded-tr-lg last:border-r-0 hover:bg-red-500 hover:text-white data-[active=true]:bg-red-500 data-[active=true]:text-white"
                        onClick={() => {
            ...
            <div className="flex flex-col gap-2">
                {uniqueKills.map((uniqueKillMethod, index) => {
                    if (
                        !uniqueKillMethod.visible ||
                        uniqueKillMethod.target !== activeTargetId
                    ) {
                        return null;
                    }
                        ...
                        >
                            ...
                            {session.data?.user?.admin && (
                                <div className="absolute top-[.9rem] right-1 flex gap-3">
                                    ...
                                    <Dialog
                                        open={deleteUniqueKillConfirmationOpen}
                                        onOpenChange={() => {
                                            setDeleteUniqueKillConfirmationOpen(
                                                false,
                                            );
